...
if __name__ == "__main__":
    ...
    weight_path = './pruning_output/mobilenet_0.906_44.6.pth'
    # state_dict = torch.load(weight_path, map_location=torch.device('cpu'))
    # mobilenet_v2.load_state_dict(torch.load(weight_path, map_location=torch.device('cpu')))
    mobilenet_v2 = torch.load(weight_path, map_location=torch.device('cpu'))
    mobilenet_v2.to('cpu')
    mobilenet_v2.eval()
    ...
    with open("./export/xnn_mobilenet.pte", "wb") as file:
