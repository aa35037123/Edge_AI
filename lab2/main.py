...
def main():
    ...
    # num_classes, train_dst, val_dst, input_size = registry.get_ (
    ...
    test_loader = torch.utils.data.DataLoader(
        dataset["test"], batch_size=args.batch_size, num_workers=0
    )
    
    # for k, v in engine.utils.utils.flatten_dict(vars(args)).items():  # print args
    #     print("%s: %s" % (k, v))

    # if args.restore is not None:
    loaded = torch.load("./mobilenetv2_0.963.pth", map_location="cpu")
    if isinstance(loaded, nn.Module):
        model = loaded
    else:
        model.load_state_dict(loaded)
    print("Loading model from {restore}".format(restore=args.restore))
